    ...
    public class BattleOperator : IBattleOperator
    {
        public string Battle(IVehicle attacker, IVehicle target)
        {
            ...
            while (!isSomeoneDead)
            {
                if (isAttackerTurn)
                {
                    targetHitPoints -= (long)Math.Max(0, attackerAttack - (targetDefense + (targetWeight / 2)));
                    isAttackerTurn = false;
                }
                else
                {
                    attackerHitPoints -= (long)Math.Max(0, targetAttack - (attackerDefense + (attackerWeight / 2)));
                    isAttackerTurn = true;
                }
